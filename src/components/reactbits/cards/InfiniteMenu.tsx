...
class InfiniteGridMenu {
  ...
  private initTexture(): void {
    ...
    // Palette of rich indigo/violet gradients for placeholders — visible immediately
    ...
    // Draw coloured gradient placeholders immediately so discs are never black
    ...
    // Then load real images and composite them over placeholders
    Promise.all(
      this.items.map(
        (item, i) =>
          new Promise<{ img: HTMLImageElement | null; index: number }>(resolve => {
            const img = new Image();
            img.crossOrigin = 'anonymous';
            img.onload = () => resolve({ img, index: i });
            img.onerror = () => resolve({ img: null, index: i }); // never block on failure
            // Add format params for better CORS + sizing compatibility
            const src = item.image.includes('unsplash.com')
              ? `${item.image.split('?')[0]}?auto=format&fit=crop&w=512&q=80&cs=srgb`
              : item.image;
            img.src = src;
          })
      )
      ...
      results.forEach(({ img, index }) => {
        if (!img) return; // failed — keep the coloured placeholder
        anyLoaded = true;
